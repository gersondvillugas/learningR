...
diamonds %>% ggplot(aes(cut,price))+
  geom_boxplot()
diamonds %>% ggplot(aes(color,price))+
  geom_boxplot()
diamonds %>% ggplot(aes(clarity,price))+
  geom_boxplot()
diamonds %>% ggplot(aes(carat,price))+
  geom_hex(bins=50)
#cuando crece tan rapido en x e y se tiende hacer logartimo para trasformarlo lienalalemnte
diamonds2<-diamonds%>% 
  filter(carat<=2.5) %>%
...
#tecnina del analisis de los residuos
diamonds2 <-diamonds2%>%
...
# lresid la parte del modelo que no queda explicado 
# en que escala estan
diamonds2 %>% ggplot(aes(cut,lresid))+
  geom_boxplot()
diamonds2 %>% ggplot(aes(color,lresid))+
  geom_boxplot()
diamonds2 %>% ggplot(aes(clarity,lresid))+
  geom_boxplot()
